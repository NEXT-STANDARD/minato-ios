...
- **Location precision (E-4b)**: the BLE broadcast carries at most a **coarse** city-level geohash (5 chars, `location.precision = "coarse"`, no `latitude`/`longitude`) — matching the app's existing public geohash-presence privacy bar. **Precise** `latitude`/`longitude` (`location.precision = "precise"`) is attached **only** to the encrypted Nostr gift-wrap directed to a contact whose active emergency override grants `safety.location.precise` (BLE MINATO is cleartext-flooded, so precise never rides BLE). Without location permission the broadcast falls back to `precision = "none"` (`undisclosed`).
