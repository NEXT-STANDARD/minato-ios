...
struct NostrTransportTests {
    ...
    func readReceiptQueueThrottlesSequentially() async throws {
        ...
        let sentFirst = await TestHelpers.waitUntil({ probe.sentEvents.count == 1 }, timeout: 5.0)
        try #require(sentFirst, "Expected first queued read receipt event")
        let scheduledThrottle = await TestHelpers.waitUntil({ probe.scheduledActionCount == 1 }, timeout: 5.0)
        ...
        let sentSecond = await TestHelpers.waitUntil({ probe.sentEvents.count == 2 }, timeout: 5.0)
